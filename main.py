...
if __name__ == '__main__':
    ...
    outputvideo = 'result.avi'
    videoname = 'test.mp4'
    capture = cv2.VideoCapture(videoname)
    fourcc      = cv2.VideoWriter_fourcc(*'XVID')
    writer      = cv2.VideoWriter(outputvideo ,fourcc, 20.0, (852,480), True)
    ...
    if capture.isOpened():
        while True:
            ...
            detected_lanes, lane_ids = net.detect(img)
            outimg = net.draw_lanes(img, detected_lanes, lane_ids)
            winName = 'Deep learning lane detection in ONNXRuntime'
            cv2.namedWindow(winName, cv2.WINDOW_NORMAL)
            cv2.imshow(winName, outimg)
            cv2.waitKey(1)
            print(img.shape)
            writer.write(outimg)	# 读取图片后一帧帧写入到视频中
